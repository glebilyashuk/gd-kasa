...
namespace WindowsFormsApp1
{
	public partial class Form2 : Form
	{
		...
		private void Get_Table(string table_name, int num_dG)
		{
			...
			OleDbDataAdapter dataAdapter = new OleDbDataAdapter(CommandText, f2.ConnectionString);
		...
		private void dataGridView4_CellContentClick(object sender, DataGridViewCellEventArgs e)
		{
			...
			label5.Text = "Поезд: №" + Convert.ToString(dataGridView4[1, row].Value) +
		...
		{
			int row;
			// Информация о кондуктор
			row = dataGridView5.CurrentCell.RowIndex;
			label6.Text = "Кондуктор: " + Convert.ToString(dataGridView5[1, row].Value);
		}
